...
type SvgPathGradientDomOutput = {
    group: SVGGElement;
    segments: SVGPathElement[];
    startCap?: SVGCircleElement;
    endCap?: SVGCircleElement;
};

type SvgPathGradientReturn = string | SvgPathGradientDomOutput;

type ColorInterpolationSpace = "srgb" | "linearRGB";

type GradientStrokePathOptions = {
    segments?: number;
    maxSegmentLength?: number; // user units
    flattenTolerance?: number; // user units (sampling step)
    colorSpace?: ColorInterpolationSpace;
    overlap?: number; // user units of arc length
    strokeWidth?: number; // used for defaults if not present in attrs
    attrs?: Record<string, string>; // applied to each segment <path>
    groupAttrs?: Record<string, string>; // applied to wrapping <g>
    decimalPlaces?: number; // coordinate rounding
...
let sharedHiddenSvg: SVGSVGElement | null = null;
let sharedHiddenPath: SVGPathElement | null = null;
...
function SvgPathGradient(pathData: string, colors: string[] | null | undefined, options: GradientStrokePathOptions = {}): SvgPathGradientReturn {
    ...
    const segmentAttributeMap: Record<string, string> = options.attrs ? { ...options.attrs } : {};
    const groupAttributeMap: Record<string, string> = options.groupAttrs ? { ...options.groupAttrs } : {};
 ...
 */
function createGroupNode(groupAttributeMap: Record<string, string>): SVGGElement {
 ...
 */
function createPathNodeFromSharedAttributes(pathData: string, stroke: string, segmentAttributeMap: Record<string, string>): SVGPathElement {
 ...
 * @throws If executed outside a browser environment (no `document`).
 */
function getSharedOffscreenPath(pathData: string): SVGPathElement {
    if (typeof document === "undefined") {
        throw new Error("SvgPathGradient: document is not available (browser-only implementation).");
    }
    ...
    if (!sharedHiddenSvg) {
        ...
        const pathElement = document.createElementNS(SvgNamespaceUri, "path");
 ...
 */
function computeMaxSegmentLength(totalLength: number, strokeWidth: number, providedMaxSegmentLength?: number): number {
 ...
 */
function computeSegmentCount(totalLength: number, providedSegments?: number, maxSegmentLength?: number): number {
 ...
 */
function buildEvenStops(rgbaColors: RgbaColor[]): { position: number; rgba: RgbaColor }[] {
    if (rgbaColors.length === 1) return [{ position: 0, rgba: rgbaColors[0] }];

    const stops: { position: number; rgba: RgbaColor }[] = new Array(rgbaColors.length);
...
function evaluateGradientAtIndex(
    stops: { position: number; rgba: RgbaColor }[],
    position: number,
    colorSpace: ColorInterpolationSpace,
    lowerStopIndex: number
): string {
 ...
 */
function interpolateColor(from: RgbaColor, to: RgbaColor, t: number, colorSpace: ColorInterpolationSpace): RgbaColor {
 ...
 */
function srgbChannelToLinear(channelByte: number): number {
 ...
 */
function normalizeOutputColor(color: RgbaColor): string {
 ...
 */
function buildSharedSegmentAttributesString(segmentAttributeMap: Record<string, string>): string {
    const fillValue = segmentAttributeMap.fill ?? "none";
 ...
 */
function buildPathElementStringFast(pathData: string, stroke: string, sharedAttributes: string): string {
 ...
 */
function buildPathElementString(input: { pathData: string; stroke: string; segmentAttributeMap: Record<string, string> }): string {
 ...
 */
function wrapGroup(pathElementStrings: string[], groupAttributeMap: Record<string, string>): string {
    const groupAttributePairs: string[] = [];
