...
vi.mock("../color-utils", () => {
    const Red: RgbaColor = { red: 255, green: 0, blue: 0, alpha: 1 };
    const Blue: RgbaColor = { red: 0, green: 0, blue: 255, alpha: 1 };

    return {
        colorsToRgba: (colors: string[]) => {
            if (colors.length <= 1) return [Red];
            const out = new Array<RgbaColor>(colors.length);
...
function installSvgPathElementPolyfill(totalLengthValue = 100) {
    ...
    if (!(globalThis as any).SVGPathElement) {
        class SVGPathElementPolyfill extends (globalThis as any).SVGElement { }
        (globalThis as any).SVGPathElement = SVGPathElementPolyfill;
    }
    ...
    vi.spyOn(document, "createElementNS").mockImplementation(
        (
            ...
            const el = originalCreateElementNS(namespace, qualifiedName, options as any);

            if (
                namespace === "http://www.w3.org/2000/svg" &&
                qualifiedName.toLowerCase() === "path"
            ) {
                const desiredProto = (globalThis as any).SVGPathElement.prototype;
...
async function loadSvgPathGradient() {
    ...
    return mod.SvgPathGradient as (
        pathData: string,
        colors: string[],
        options?: any
    ) => string | { group: SVGGElement; segments: SVGPathElement[] };
}
...
describe("SvgPathGradient (only public function behavior)", () => {
    ...
    it("throws when pathData is empty or not a string", async () => {
        ...
        expect(() => SvgPathGradient(null as unknown as string, ["#f00"])).toThrow(/pathData must be a non-empty string/i);
    });
    ...
    it("throws when colors is empty or not an array", async () => {
        ...
        expect(() => SvgPathGradient("M 0 0 L 10 0", null as unknown as string[])).toThrow(/colors must be a non-empty array/i);
    });
    ...
    it("returns DOM output in dom mode with a group and segments", async () => {
        ...
        const dom = out as { group: SVGGElement; segments: SVGPathElement[] };
